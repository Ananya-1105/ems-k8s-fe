...
const ManageEmployees = () => {
  ...
  const fetchData = async () => {
    try {
      const [empRes, deptRes] = await Promise.all([
        axios.get("http://localhost:30083/api/employees"),
        axios.get("http://localhost:30083/api/departments"),
      ]);
  ...
  const saveNewEmployee = async () => {
    try {
      await axios.post("http://localhost:30083/api/employees", {
        ...formData,
  ...
  const saveEdit = async () => {
    try {
      await axios.put(`http://localhost:30083/api/employees/${editEmployeeId}`, {
        ...formData,
        salary: parseFloat(formData.salary),
        departmentId: formData.departmentId ? parseInt(formData.departmentId) : null,
        hireDate: formData.hireDate,
  ...
  const deleteEmployee = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:30083/api/employees/${id}`);
      fetchData();
    } catch (err) {
